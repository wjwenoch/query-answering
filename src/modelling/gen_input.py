...
def base_model():
    """
    TODO: Add Query Representation layer using hierarchical RNNs/CNNs.
    :return:
    """
...
def get_inout(inds):
    ...
    for k, v in inds.items():
        ...
        print(v['mat_ca'].shape)
    in_ras = np.array(in_ras)
    print("in_ras.shape", in_ras.shape)
    in_das = np.array(in_das)
    out_cas = np.array(out_cas)
    print("out_cas", out_cas.shape)
    return ([in_ras, in_das], out_cas)


def test_model(mf, tbox):
    testdir = '/projects/PycharmProjects/query-answering/src/files/lubm/data/test'
